...
from util.dataloader_med import RetinaDataset, Augmentation, ChestX_ray14, Covidx, CheXpert,PediCXRDataset
from util.custom_transforms import GaussianBlur
import torch
from util.augment import new_data_aug_generator
...
def build_dataset_chest_xray(split, args):
    ...
    elif args.dataset == 'PediCXR':
        print(args.dataset)
        dataset = PediCXRDataset(data_dir=args.data_path, phase=split, transform=transform)
...
def build_transform(is_train, args):
    ...
    if args.norm_stats is not None:
        ...
        try:
            if args.dataset == 'chestxray' or args.dataset == 'covidx' or args.dataset == 'chexpert' or args.dataset == 'PediCXR':
                mean = (0.5056, 0.5056, 0.5056)
                std = (0.252, 0.252, 0.252)
...
def get_args_parser():
    ...
    parser.add_argument('--data_path', default='/nfs/turbo/med-kayvan-lab/Projects/HeartFailure/Data/Raw/physionet.org/files/vindr-pcxr/1.0.0', type=str,
                        help='dataset path')
    parser.add_argument("--dataset", default='PediCXR', type=str)
    parser.add_argument("--build_timm_transform", action='store_true', default=False)
    return parser
def main(args):
    dataset = build_dataset_chest_xray('test', args)
    print(len(dataset))
    sample = dataset[0]
...
if __name__ == '__main__':
    ...
    # dataset = PediCXRDataset(
    #     data_dir = '/nfs/turbo/med-kayvan-lab/Projects/HeartFailure/Data/Raw/physionet.org/files/vindr-pcxr/1.0.0',
    #     phase = 'test',
    #     transform,
    #     merge_disease = True):
